...
def create_loc_conf(num_classes=81, bbox_aspect_num=[4, 6, 6, 6, 4, 4]):
    '''
    Create location and confidence layers
    Args:
        num_classes: int, number of classes
        bbox_aspect_num: list, number of aspect ratios for each cell of source1->source6
    '''
    loc_layers = []
    conf_layers = []
...
class SSD(nn.Module):
    ...
    def forward(self, x):
        ...
        conf = conf.view(conf.size(0), -1, self.num_classes) #(batch_size, 8732, 81)
